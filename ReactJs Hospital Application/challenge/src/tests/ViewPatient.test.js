...
describe("testing ViewPatient", () => {
  ...
  beforeEach(
    () => (wrapper = shallow(<ViewPatient match={{ params: { id: 1 } }} />))
  );
  {
  ...
  it("renders a name input", () => {
    expect(
      shallow(<ViewPatient match={{ params: { id: 1 } }} />).find("#name")
        .length
    ).toEqual(1);
  });
  it("renders a email", () => {
    expect(
      shallow(<ViewPatient match={{ params: { id: 1 } }} />).find("#email")
        .length
    ).toEqual(1);
  });
  it("renders a dob", () => {
    expect(
      shallow(<ViewPatient match={{ params: { id: 1 } }} />).find("#dob").length
    ).toEqual(1);
  });
  it("renders a location", () => {
    expect(
      shallow(<ViewPatient match={{ params: { id: 1 } }} />).find("#location")
        .length
    ).toEqual(1);
  });
  it("renders a mobile", () => {
    expect(
      shallow(<ViewPatient match={{ params: { id: 1 } }} />).find("#mobile")
        .length
    ).toEqual(1);
  });
});
