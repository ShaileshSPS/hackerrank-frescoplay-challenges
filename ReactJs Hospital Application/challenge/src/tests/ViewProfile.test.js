...
describe("testing ViewProfile", () => {
  ...
  beforeEach(
    () =>
      (wrapper = shallow(<ViewProfile match={{ params: { adminId: 1 } }} />))
  );
  ...
  it("renders a name input", () => {
    expect(
      shallow(<ViewProfile match={{ params: { adminId: 1 } }} />).find("#name")
        .length
    ).toEqual(1);
  });
  it("renders a email", () => {
    expect(
      shallow(<ViewProfile match={{ params: { adminId: 1 } }} />).find("#email")
        .length
    ).toEqual(1);
  });
  it("renders a dob", () => {
    expect(
      shallow(<ViewProfile match={{ params: { adminId: 1 } }} />).find("#dob")
        .length
    ).toEqual(1);
  });
  it("renders a location", () => {
    expect(
      shallow(<ViewProfile match={{ params: { adminId: 1 } }} />).find(
        "#mobileno"
      ).length
    ).toEqual(1);
  });
