...
describe("testing EditPatient", () => {
  ...
  beforeEach(
    () => (wrapper = shallow(<EditPatient match={{ params: { id: 1 } }} />))
  );
  ...
  it("renders a name input", () => {
    expect(
      shallow(<EditPatient match={{ params: { id: 1 } }} />).find("#name")
        .length
    ).toEqual(1);
  });
  it("renders a email", () => {
    expect(
      shallow(<EditPatient match={{ params: { id: 1 } }} />).find("#email")
        .length
    ).toEqual(1);
  });
  it("renders a dob", () => {
    expect(
      shallow(<EditPatient match={{ params: { id: 1 } }} />).find("#dob").length
    ).toEqual(1);
  });
  it("renders a location", () => {
    expect(
      shallow(<EditPatient match={{ params: { id: 1 } }} />).find("#location")
        .length
    ).toEqual(1);
  });
  it("renders a mobile", () => {
    expect(
      shallow(<EditPatient match={{ params: { id: 1 } }} />).find("#mobile")
        .length
    ).toEqual(1);
  });
  ...
  it("should change the dob state of adding patient", () => {
    wrapper
      .find("#dob")
      .simulate("change", { target: { name: "dob", value: "26/05/1996" } });
    expect(wrapper.state("dob")).toEqual("26/05/1996");
  });
  it("should change the location state of adding patient", () => {
    wrapper
      .find("#location")
      .simulate("change", { target: { name: "location", value: "Chennai" } });
  ...
  it("should change the mobile state of adding patient", () => {
    wrapper
      .find("#mobile")
      .simulate("change", { target: { name: "mobile", value: "8667879989" } });
